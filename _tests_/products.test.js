...
beforeAll(async () => {
    // Aquí puedes realizar configuraciones previas a las pruebas, como inicializar la base de datos o cargar datos de prueba.
    const loginResponse = await request(app).post('/api/auth/login').send({
        email: 'user@email.com',
        password: 'admin'
    });

    token = loginResponse.body.token; // Guardar el token para usarlo en las pruebas
});
